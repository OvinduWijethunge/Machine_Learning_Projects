...
df = pd.read_csv('comments1.csv',encoding='utf-16') 

li_id = df['comment_id'].tolist()

number_of_round = df_con.shape[0]
...
fields_list = []
for x in range(number_of_round):
    print("round????????????????????????????????????????????????????????????????????????????????????????? ")
    #vid = worksheet1.cell(row=x, column=2).value
    vid = df_con.loc[x,'vid']
    main_list = []
    post_date = ""
    ...
    content_string = df_con.loc[x,'contents']
...
#---------------------------------------------------------------------------------------
    ...
    center_vector = np.zeros((len(vectors_list[1])))
